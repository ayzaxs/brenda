...
function renderizarCartas(): void {
    ...
    if (!containerHoje || !containerPassadas) return;
    ...
    const cartasDisponiveis = minhasCartas.filter(carta => {
        if (carta.dataLiberacao < hojeStr) return true;
        if (carta.dataLiberacao === hojeStr && horaAtual >= 18) return true;
        return false;
    });
    ...
    cartasDisponiveis.forEach((carta, index) => {
        ...
        const imagemHTML = carta.imagemUrl ? `
            <img src="${carta.imagemUrl}" alt="Lembrança especial" class="carta-imagem">
        ` : '';
        
        // Montagem final da carta com todos os elementos
        const cartaHTML = `
